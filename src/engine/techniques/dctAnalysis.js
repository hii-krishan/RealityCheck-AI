...
export function performDCTAnalysis(imageData, width, height) {
  ...
  const quantizedDC = dcValues.map(v => Math.round(v));
  const dcHistogram = {};
  ...
  // Very low high-frequency content suggests AI generation
  if (meanHFR < 0.05) score += 35;
  else if (meanHFR < 0.1) score += 20;
  else if (meanHFR < 0.15) score += 10;
  
  // Unusual AC energy variance suggests manipulation
  if (cvAC > 3) score += 25;
  else if (cvAC > 2) score += 15;
  else if (cvAC > 1.5) score += 8;
  
  // Double compression indicators
  if (periodicGaps > 10) score += 30;
  else if (periodicGaps > 5) score += 20;
  else if (periodicGaps > 2) score += 10;
  
  // Very uniform AC energy (AI tendency)
  if (cvAC < 0.3 && meanHFR < 0.15) score += 15;
  ...
  return {
    ...
    confidence,
    details,
    stats: { meanHFR: (meanHFR * 100).toFixed(1) + '%', cvAC: cvAC.toFixed(3), periodicGaps, blocksAnalyzed: acEnergies.length }
  };
}
